...
const SButton = styled.button<IButtonStyleProps>`
  ...
  background-color: ${({ outline, color }) => (outline ? "transparent" : `rgb(${colors["white"]})`)};
  border: ${({ outline, color }) => (outline ? `1px solid rgb(${colors["white"]})` : "none")};
  color: ${({ outline, color }) => (outline ? `rgb(${colors["black"]})` : `rgb(${colors.black})`)};
  ...
  &:active {
    ...
    box-shadow: ${({ outline }) => (outline ? "none" : `${shadows.soft}`)};
    color: ${({ outline, color }) =>
      outline ? `rgb(${colors["white"]})` : `rgba(${colors.white}, 0.24)`};
